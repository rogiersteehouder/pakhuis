...
class ConfigReader:
    ...
    def get_value(self, val: Any, target: type, name: str | None = None) -> Any:
...
#     def get_value(self, val: Any, target: type, name: str) -> Any:  # noqa: PLR0911
#         """Produce the correct value for a setting."""
#         # UnionType: assume the first one (unless None)
#         # this accepts a type hint of "SomeClass | None" and do the right thing
#         if isinstance(target, UnionType):
...
#             fn, section = data["_include"].partition(":")
#             data = ConfigReader.from_file(target, self.cfg_file.parent / fn, name)
#             if section:
#                 data = data[section]
...
#         if len(data) == 1 and "_include" in data:
#             fn, section = data["_include"].partition(":")
#             new_data = ConfigReader.from_file(target, self.cfg_file.parent / fn, name)
#             return new_data[section] if section else new_data
